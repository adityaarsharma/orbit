...
Every Release invocation runs **every skill in the Skill commands block below**, end-to-end. The 7 release-gate checks are MINIMUM, not maximum — all i18n / runtime-trap / security / perf / docs / changelog / wp-org-gate skills also run. A release blocked by any one of them stays blocked. Opt-out requires a skip reason recorded in the run report. Build the work-list via `TaskCreate`. End with a Coverage Report.
...
```
Read the relevant skill files under `skills/` (release-gate, plugin-check,
release-meta, changelog, version-compare, zip-hygiene, i18n, ip-cleanroom),
the checklists under `checklists/`, and this agent's own Skills list above.
No external brain — everything needed to run the release gate is in the repo.

CHECK: latest WP release? Update "Tested up to" if needed.
LOAD: past rejection reasons from the run report → verify those specific issues are fixed.
```
...
CHECK 3: orbit-plugin-check (WP.org Plugin Check — runtime-evergreen)
  ...
  EXCEPTION: if previously approved by operator:
    → run report has [release, <plugin>, plugin-check-exception, <reason>]
...
CHECK 8: orbit-ip-cleanroom (IP / copyright clean-room — hard gate)
  ...
  (orbit-security owns the scan; release confirms the gate. If no reference was ever used,
   record [release, <plugin>, ip-cleanroom-NA, "no reference studied"] in the run report and pass.)
  ...
  STOP: premium/obfuscated reference was decompiled → escalate to legal (see security RUNBOOK in the repo),
        do NOT ship
```
...
```
Publish release notes / announce via your own channels (gh release, manual):
  → gh release create v<version> with the release notes
  → Post the announcement to whatever channels you run (manual)
  → Comment summary: "🚀 v<version> released — <top 2 user benefits>"
```

### Step 8 — Record in the run report
...
🚫 NEVER announce without operator approve on the release
✅ ALWAYS create a release PR before merging
✅ ALWAYS get operator approval before publishing
✅ ALWAYS create a git tag after merge
✅ ALWAYS record the release in the run report when it ships
```
...
|---|---|---|
| `gh` CLI | Create PR, create git tag, push | GitHub login |
