...
- **E7 bulk admin action at scale (fatal/timeout — invisible on single-item tests):** WordPress admin bulk actions (**Comments / Posts / Users → select many → Trash / Delete / Edit / Approve**) fire the per-item hooks (`delete_comment`/`trash_comment`/`trashed_comment`/`transition_comment_status`/`wp_set_comment_status`; `save_post`/`before_delete_post`/`wp_trash_post`) **N times in one request**. A plugin hooked on these must: (a) survive a **missing/null parent object** — e.g. a comment whose post is already trashed → `get_post()` returns null → `$post->ID` fatals; (b) **batch or defer** expensive work (cache purge, remote CDN/API call, `$wpdb` writes) instead of doing it per-item; (c) not exhaust **memory/time** across the batch. The classic fatal is a cache-purge or event hook calling a method on a `null` `WP_Post`/`WP_Comment` during a bulk trash. **Detect:** any callback on the bulk-fired comment/post events doing heavy or unguarded work; also `handle_bulk_actions-*` filters and `wp_ajax_*` mass handlers. **Test live with a 50–200 item bulk action, never a single item** — single-item CI passes while the bulk path fatals. (Cross-ref G4/G6: cache plugins purge on comment/post events, so they are the most common trippers.)
...
## Category G — Full-Page Cache & Drop-in Plugins
Cache plugins carry unique, high-blast-radius risk: they run code **before WordPress loads** (`advanced-cache.php`, `object-cache.php`), **edit `wp-config.php`**, and **write request-influenced paths to disk**. One mistake WSODs every page or serves one user's private page to another. Weight these whenever the plugin ships an `advanced-cache.php`/`object-cache.php` drop-in, defines `WP_CACHE`, or writes `wp-content/cache`.
- **G1 wp-config.php programmatic edit:** code writing `wp-config.php` MUST (a) read the real current file, (b) target ONLY the intended constant via an anchored regex, (c) create a backup, (d) restore-on-failure, (e) verify `is_writable` first. Anti-pattern: rebuilding config from a template/string; no backup; greedy regex spanning newlines (missing `/m`, `.*` w/o care). A corrupted `wp-config.php` = total site death. Edge: multisite; `wp-config.php` one dir above ABSPATH; read-only/immutable configs; define already present with odd spacing.
- **G2 pre-WP drop-in path from request (traversal):** the drop-in runs before WP sanitization exists. Cache paths built from `REQUEST_URI`/`HTTP_HOST`/query MUST collapse `..`, reject encoded `%2e%2e`, allowlist WITHOUT passing `.`/`/` through, or `realpath()`-contain to the cache dir. Edge: literal `../` (server-normalization-dependent — don't rely on it), `HTTP_HOST` used as a path segment unsanitized, null byte, over-long path, mixed-case encoded.
- **G3 cache-write must gate on 200 + bypass constants:** only cache `http_response_code()===200`; honor `DONOTCACHEPAGE`, `DONOTCACHEOBJECT`, `Cache-Control: no-store|no-cache|private`, and `Set-Cookie`. Anti-pattern: caching 404/301/302/500, caching authenticated responses, ignoring `Vary`. A cached redirect loop or error page is a top support complaint.
- **G4 logged-in / private-content bleed (cache poisoning):** the cache key MUST vary on auth state (logged-in cookie) AND on the vary axes the response actually uses (device mobile/desktop, currency/lang cookie, query allow/ignore list). Missing → one user served another's cart/account page, or a nonce cached and shared. The single most damaging cache-bug class.
- **G5 nonce/CSRF token cached:** full-page HTML containing WordPress nonces cached and served to many users → nonces mismatch/expire → forms & AJAX break site-wide (or a valid nonce leaks). Detect: caching pages containing `wp_nonce_field`/`_wpnonce` without excluding them; cache TTL >> nonce lifetime (12–24h).
- **G6 orphaned drop-in on deactivate/uninstall:** deactivation/uninstall MUST remove `advanced-cache.php`, `object-cache.php`, the `WP_CACHE` define, and the cache dir. A leftover drop-in referencing now-deleted plugin classes = **fatal on every request after the plugin is deleted** (the #1 cache-plugin "white screen" support ticket). Verify `register_deactivation_hook` + `uninstall.php` clean ALL drop-ins.
- **G7 object-cache unserialize without allowed_classes:** `object-cache.php` reading backend values (Redis/Memcached) via `unserialize()` without `['allowed_classes'=>false]` → object injection if the store is shared/compromised. Prefer `allowed_classes=>false` or a safe serializer.
- **G8 cache dir web-exposure:** disk cache dir must ship `index.php` + `.htaccess` deny (and `web.config` for IIS); cached HTML can contain private data. Detect: cache write without a protecting index/deny file in the dir.
- **G9 cache-write concurrency / atomicity:** concurrent requests writing the same cache file → torn/partial reads. Prefer write-temp-then-`rename()` (atomic). Edge: stampede on a popular URL right after purge (thundering herd) with no lock.
- **G10 cache-file explosion / inode exhaustion:** unbounded cache keys (every UTM/query permutation a new file) → millions of files, inode/disk exhaustion, slow purge-all. Require a query allow/ignore list and a bounded key space.
- **G11 direct filesystem instead of WP_Filesystem (runtime):** outside the pre-WP drop-in, file writes should use `WP_Filesystem` (WP.org review flags raw `fopen`/`file_put_contents` in admin/runtime code). The drop-in itself is the documented exception (WP not loaded — annotate it).
- **G12 gzip double-compression / encoding mismatch:** caching a gzipped body then letting the server gzip again, or serving `.gz` without matching `Content-Encoding`/`Vary: Accept-Encoding` → garbled pages for some clients.
